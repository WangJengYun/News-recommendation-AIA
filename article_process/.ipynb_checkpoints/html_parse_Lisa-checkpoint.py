...
if __name__ == "__main__":
    ...
    # -----------Lisa Add -------------------
    Info_filename = "./sysjust03/NEWS_2nd/news_info.txt"
    Class_Info = []
    with open(Info_filename) as f:
        for line in f:
            Class_Info.append([item for item in line.strip().split("|")])
    # -----------Lisa Add -------------------
...
#         reading text and data preparation
        ...
        for index in range(len(Class_Info)):
            if Class_Info[index][0] in news_record['new_id']:
                class_info = Class_Info[index][2]
                news_date = Class_Info[index][1]
                DelIndex = index
                break
        if DelIndex>-1:
            del Class_Info[DelIndex]
        # -----------Lisa Add -------------------
